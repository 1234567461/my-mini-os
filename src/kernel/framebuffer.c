/* ==========================================
 * Framebuffer - 帧缓冲驱动 v0.8.0
 * 功能：VGA图形模式下的帧缓冲操作
 * ========================================== */

#include "gui.h"
#include "types.h"
#include "vga.h"

/* 帧缓冲信息 */
static uint32_t *framebuffer = (uint32_t *)0xE0000000;  /* 线性帧缓冲基址 */
static int fb_initialized = 0;

/* 当前剪辑区域 */
static rect_t clip_rect = {0, 0, GUI_WIDTH, GUI_HEIGHT};

/* ==========================================
 * 初始化帧缓冲
 * ========================================== */
void framebuffer_init(void) {
    /* 在真实的硬件上，这里需要：
     * 1. 设置VGA图形模式 (mode 0x118 = 640x480x256 or 0x12 = 640x480x16)
     * 2. 获取帧缓冲地址
     * 3. 设置合适的pitch和bpp
     *
     * 目前使用预定义的地址，实际运行时会通过VESA获取
     */

    fb_initialized = 1;

    /* 清空帧缓冲 */
    framebuffer_fill_rect(0, 0, GUI_WIDTH, GUI_HEIGHT, COLOR_BLACK);
}

/* ==========================================
 * 设置像素
 * ========================================== */
void framebuffer_set_pixel(int x, int y, uint32_t color) {
    if (!fb_initialized) return;

    /* 边界检查 */
    if (x < 0 || x >= GUI_WIDTH || y < 0 || y >= GUI_HEIGHT) return;

    /* 剪辑区域检查 */
    if (x < clip_rect.x || x >= clip_rect.x + clip_rect.width ||
        y < clip_rect.y || y >= clip_rect.y + clip_rect.height) return;

    framebuffer[y * GUI_WIDTH + x] = color;
}

/* ==========================================
 * 获取像素
 * ========================================== */
uint32_t framebuffer_get_pixel(int x, int y) {
    if (!fb_initialized) return 0;
    if (x < 0 || x >= GUI_WIDTH || y < 0 || y >= GUI_HEIGHT) return 0;

    return framebuffer[y * GUI_WIDTH + x];
}

/* ==========================================
 * 填充矩形
 * ========================================== */
void framebuffer_fill_rect(int x, int y, int width, int height, uint32_t color) {
    if (!fb_initialized) return;

    /* 边界裁剪 */
    int start_x = x < 0 ? 0 : x;
    int start_y = y < 0 ? 0 : y;
    int end_x = (x + width) > GUI_WIDTH ? GUI_WIDTH : (x + width);
    int end_y = (y + height) > GUI_HEIGHT ? GUI_HEIGHT : (y + height);

    /* 填充 */
    for (int py = start_y; py < end_y; py++) {
        for (int px = start_x; px < end_x; px++) {
            framebuffer[py * GUI_WIDTH + px] = color;
        }
    }
}

/* ==========================================
 * 像素块传输
 * ========================================== */
void framebuffer_blit(int x, int y, int width, int height, uint32_t *data) {
    if (!fb_initialized || data == NULL) return;

    for (int py = 0; py < height; py++) {
        for (int px = 0; px < width; px++) {
            int dest_x = x + px;
            int dest_y = y + py;
            if (dest_x >= 0 && dest_x < GUI_WIDTH && dest_y >= 0 && dest_y < GUI_HEIGHT) {
                framebuffer[dest_y * GUI_WIDTH + dest_x] = data[py * width + px];
            }
        }
    }
}

/* ==========================================
 * 区域复制
 * ========================================== */
void framebuffer_copy(int dest_x, int dest_y, int src_x, int src_y, int width, int height) {
    if (!fb_initialized) return;

    /* 从下往上复制，避免覆盖 */
    if (dest_y < src_y) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                int sx = src_x + x;
                int sy = src_y + y;
                int dx = dest_x + x;
                int dy = dest_y + y;
                if (sx >= 0 && sx < GUI_WIDTH && sy >= 0 && sy < GUI_HEIGHT &&
                    dx >= 0 && dx < GUI_WIDTH && dy >= 0 && dy < GUI_HEIGHT) {
                    framebuffer[dy * GUI_WIDTH + dx] = framebuffer[sy * GUI_WIDTH + sx];
                }
            }
        }
    } else {
        for (int y = height - 1; y >= 0; y--) {
            for (int x = 0; x < width; x++) {
                int sx = src_x + x;
                int sy = src_y + y;
                int dx = dest_x + x;
                int dy = dest_y + y;
                if (sx >= 0 && sx < GUI_WIDTH && sy >= 0 && sy < GUI_HEIGHT &&
                    dx >= 0 && dx < GUI_WIDTH && dy >= 0 && dy < GUI_HEIGHT) {
                    framebuffer[dy * GUI_WIDTH + dx] = framebuffer[sy * GUI_WIDTH + sx];
                }
            }
        }
    }
}

/* ==========================================
 * 刷新帧缓冲（用于双缓冲）
 * ========================================== */
void framebuffer_flush(void) {
    /* 在真实硬件上，这里可能需要：
     * 1. 等待垂直回扫
     * 2. 复制到硬件帧缓冲
     *
     * 目前是直接模式，无需额外操作
     */
}